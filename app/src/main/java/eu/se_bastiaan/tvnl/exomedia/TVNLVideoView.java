...
import eu.se_bastiaan.tvnl.exomedia.builder.RenderBuilder;
import eu.se_bastiaan.tvnl.exomedia.builder.SmoothStreamingRenderBuilder;
...
public class TVNLVideoView extends RelativeLayout implements AudioCapabilitiesReceiver.Listener {
    private static final String TAG = TVNLVideoView.class.getSimpleName();
     ...
     *
     * Public so that it can be overriden by people that need more advanced
     *
    ...
    private RenderBuilder getRendererBuilder(VideoType renderType, Uri uri, MediaUtil.MediaType defaultMediaType) {
        switch (renderType) {
            ...
            case SMOOTH_STREAM:
                return new SmoothStreamingRenderBuilder(getContext().getApplicationContext(), getUserAgent(), uri.toString());
            default:
    ...
    public String getUserAgent() {
        return String.format(Locale.US, "NPOxUG/%s AndroidTV/%s %s/%s", eu.se_bastiaan.tvnl.BuildConfig.VERSION_NAME, Build.VERSION.RELEASE, Build.MANUFACTURER, Build.MODEL);
    }
