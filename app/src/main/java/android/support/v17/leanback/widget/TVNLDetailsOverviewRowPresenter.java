 ...
 * DetailsFragment}.  The View created by the TVNLDetailsOverviewRowPresenter is made in three parts:
 ...
 * {@link #TVNLDetailsOverviewRowPresenter(Presenter)}.  Typically this will be an instance of
 ...
 *
 * <p>
 * Because transition support and layout are fully controlled by TVNLDetailsOverviewRowPresenter,
 * developer can not override TVNLDetailsOverviewRowPresenter.ViewHolder for adding/replacing views
 * of TVNLDetailsOverviewRowPresenter.  If further customization is required beyond replacing
 * the detailed description, the application should create a new row presenter class.
 * </p>
 */
public class TVNLDetailsOverviewRowPresenter extends RowPresenter {

    private static final long DEFAULT_TIMEOUT = 5000;

    class ActionsItemBridgeAdapter extends ItemBridgeAdapter {
        TVNLDetailsOverviewRowPresenter.ViewHolder mViewHolder;

        ActionsItemBridgeAdapter(TVNLDetailsOverviewRowPresenter.ViewHolder viewHolder) {
    ...
    public final class ViewHolder extends RowPresenter.ViewHolder {
        ...
        final OnChildViewHolderSelectedListener mChildViewHolderSelectedListener = new OnChildViewHolderSelectedListener() {
            @Override
            public void onChildViewHolderSelected(RecyclerView parent, RecyclerView.ViewHolder child, int position, int subposition) {
                dispatchItemSelection((ItemBridgeAdapter.ViewHolder) child);
            }
        };

        void dispatchItemSelection(ItemBridgeAdapter.ViewHolder viewHolder) {
            if (!isSelected()) {
                return;
            }
            ItemBridgeAdapter.ViewHolder ibvh = (ItemBridgeAdapter.ViewHolder) (viewHolder != null ?
                    viewHolder :
                    mActionsRow.findViewHolderForAdapterPosition(mActionsRow.getSelectedPosition()));
            if (ibvh == null) {
        ...
        private void checkFirstAndLastPosition(boolean fromScroll) {
            ...
            viewHolder = mActionsRow.findViewHolderForAdapterPosition(mNumItems - 1);
            boolean showRight = (viewHolder == null ||
                    viewHolder.itemView.getRight() > mActionsRow.getWidth());

            viewHolder = mActionsRow.findViewHolderForAdapterPosition(0);
        ...
        public ViewHolder(View rootView, Presenter detailsPresenter) {
            ...
            mActionsRow.setAdapter(mActionBridgeAdapter);
            mActionsRow.setOnChildViewHolderSelectedListener(mChildViewHolderSelectedListener);
    ...
    private TVNLDetailsOverviewSharedElementHelper mSharedElementHelper;

    /**
     * Constructor for a TVNLDetailsOverviewRowPresenter.
     *
     * @param detailsPresenter The {@link Presenter} used to render the detailed
     *        description of the row.
     */
    public TVNLDetailsOverviewRowPresenter(Presenter detailsPresenter) {
    ...
    public final void setSharedElementEnterTransition(Activity activity,
        ...
        if (mSharedElementHelper == null) {
            mSharedElementHelper = new TVNLDetailsOverviewSharedElementHelper();
        }
    ...
    protected RowPresenter.ViewHolder createRowViewHolder(ViewGroup parent) {
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.details_overview, parent, false);
